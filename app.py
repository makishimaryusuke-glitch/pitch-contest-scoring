...
# 採点ワークフロー（提出資料のアップロード、AI採点の実行、採点結果を統合）
elif page == "📝 採点ワークフロー":
    st.title("📝 採点ワークフロー")
    
    # タブで3つの機能を切り替え
    tab1, tab2, tab3 = st.tabs(["📤 提出資料のアップロード", "🤖 AI採点の実行", "📊 採点結果"])
    
    # タブ1: 提出資料のアップロード
    with tab1:
        st.subheader("📤 提出資料のアップロード")
        ...
        with col1:
            selected_school = st.selectbox("参加校を選択", ["新規作成"] + list(school_options.keys()), key="upload_school_select")
        
        if selected_school == "新規作成":
            with col2:
                st.subheader("新規参加校を登録")
                new_school_name = st.text_input("学校名 *", key="new_school_name")
                new_prefecture = st.text_input("都道府県", key="new_prefecture")
                
                if st.button("参加校を登録", key="register_school"):
                    if new_school_name:
        ...
        if selected_school != "新規作成":
            ...
            if st.button("提出資料を登録", disabled=not (theme_title and uploaded_files), key="register_submission"):
    ...
    # タブ2: AI採点の実行
    with tab2:
        st.subheader("🤖 AI採点の実行")
        
        submissions = get_all_submissions()
        if not submissions:
            st.info("提出資料がありません。まず「提出資料のアップロード」タブで資料を登録してください。")
        else:
            submission_options = {f"{s.get('school_name', '不明')} - {s['theme_title']}": s['id']
                                 for s in submissions}
            selected_submission = st.selectbox("採点する提出資料を選択", list(submission_options.keys()), key="scoring_submission_select")
            ...
            if selected_submission:
                ...
                if submission:
                    ...
                    # 採点実行
                    if st.button("AI採点を実行", type="primary", disabled=not is_api_configured(), key="execute_scoring"):
    ...
    with tab3:
        ...
        if not completed_results:
            st.info("まだ採点結果がありません。まず「AI採点の実行」タブで採点を実行してください。")
        else:
            # フィルタリング
            col1, col2 = st.columns(2)
            with col1:
                school_names = list(set(r.get("school_name", "不明") for r in completed_results))
                school_filter = st.selectbox("学校でフィルタ", ["すべて"] + school_names, key="result_school_filter")
            with col2:
                sort_option = st.selectbox("並び替え", ["スコア順（高い順）", "スコア順（低い順）", "日付順（新しい順）"], key="result_sort")
            ...
            # エクスポート
            st.subheader("エクスポート")
            if st.button("CSV形式でエクスポート", key="export_csv"):
